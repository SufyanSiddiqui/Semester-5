...
{
	int no_of_tasks,T_id, no_of_workers,destination,mtag,rows,index,i,j,k; 	
	int a[SIZE][SIZE] = {{1,2,3},{5,6,7},{8,9,10}};  
	...
	MPI_Comm_size(MPI_COMM_WORLD,&no_of_tasks);
	if (no_of_tasks < 2 ) 
	{
	  printf("No_of_task must be greater than 2\n");
	  MPI_Abort(MPI_COMM_WORLD, 0);
	  exit(1);
	}

	no_of_workers = no_of_tasks-1;
	   ...
	   if (T_id == ROOT_PROCESS)
	   {
	      ...
	      {
		 rows =1;  	
		 MPI_Send(&index, 1, MPI_INT, destination, mtag, MPI_COMM_WORLD);
		 MPI_Send(&rows, 1, MPI_INT, destination, mtag, MPI_COMM_WORLD);
